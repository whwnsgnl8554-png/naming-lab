...
function 한자가중치(h, ctx) {
  ...
  // 인기 트렌드 가중 — 한자의 음(소리)이 인기 이름 음절 풀에 있으면 강한 가산
  // 예: 도/윤/서/준/시/우/하/이 (남), 서/아/하/윤/지/유 (여)
  if (ctx.인기트렌드 && ctx.인기성별 && ctx.인기성별 !== 'u') {
    const syl = ctx.인기성별 === 'm' ? POP_SYL_M : POP_SYL_F;
    if (syl.has(h.음)) w += 14;     // 가장 인기 음절에 매칭 시 +14
  }
...
export function 인물작명(input) {
  const {
    ...
    합성어순 = '아빠먼저',
    옛스러움제외 = true,
    인기트렌드 = true,        // 출생연도 기준 인기 이름 가중치 (디폴트 켜짐)
    이름지정 = false,
    지정이름 = '',
    // ── 외국인 귀화 모드 ──
    외국인모드 = false,
  ...
  for (let tries = 0; tries < 12 && 후보들.length < 5; tries++) {
    ...
    if (음절 === 1) {
      ...
      for (const h of 첫) {
        ...
        후보들.push(이름카드만들기({ 첫: h, 둘: null, 성, 한글, 사주, 띠정보, 키워드, 성별, 출생연도: 생년, 인기트렌드 }));
      ...
      for (const c of 조합) {
        ...
        if (이미존재(후보들, 한글)) continue;
        const 카드 = 이름카드만들기({ 첫: c.첫, 둘: c.둘, 성, 한글, 사주, 띠정보, 키워드, 성별, 출생연도: 생년, 인기트렌드 });
...
function 이름카드만들기({ 첫, 둘, 성, 한글, 사주, 띠정보, 키워드, 성별, 출생연도, 인기트렌드 }) {
  ...
  // 인기 이름 매칭 — 그 이름의 시대·순위 뱃지 + 종합점수 가산
  let 인기 = null;
  if (성별 && 성별 !== 'u') {
    인기 = 인기랭킹(한글, 성별);
    if (인기트렌드 && 인기) {
      const 가산 = Math.round(인기가중(한글, 성별, 출생연도) * 0.6);
      종합 = Math.min(100, 종합 + 가산);
    }
  }
...
function 지정이름작명({ 성, 성별, 지정이름, 한자사용, 사주, 띠정보, 키워드, 부족오행, 충만오행, 옛스러움제외, 보강운세 }) {
  ...
  if (음절.length === 1) {
    ...
    for (const { h } of 음절후보[0]) {
      ...
      seen.add(h.자);
      후보들.push(이름카드만들기({ 첫: h, 둘: null, 성, 한글: h.음, 사주, 띠정보, 키워드, 성별, 인기트렌드 }));
    ...
    for (const { h: a } of 음절후보[0]) {
      for (const { h: b } of 음절후보[1]) {
        ...
        seen.add(key);
        const 카드 = 이름카드만들기({ 첫: a, 둘: b, 성, 한글: a.음 + b.음, 사주, 띠정보, 키워드, 성별, 인기트렌드 });
        if (옛스러움제외 && 카드.시대성 < 50) continue;
        후보들.push(카드);
        if (후보들.length >= 6) break;
      }
  ...
  if (한자사용) {
    ...
    for (const 첫 of 첫S.slice(0, 6)) {
      for (const 둘 of 둘S.slice(0, 6)) {
        ...
        if (seen.has(key)) continue;
        seen.add(key);
        후보들.push(이름카드만들기({ 첫, 둘, 성, 한글, 사주, 띠정보, 키워드, 성별, 인기트렌드 }));
        if (후보들.length >= 6) break;
      }
      if (후보들.length >= 6) break;
    }
